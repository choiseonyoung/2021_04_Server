...
/*
 * 이번 프로젝트는 1개의 table만으로 구성된 간단한 프로젝트이므로
 * 별도로 vo, dto를 만들지 않고
 * Map<>을사용하여 구성한다.
 * 
 * Map은 key, value 형식의 데이터를 저장하는 자료구조
 * put(key,value) method를 사용하여 데이터항목(컬럼)을 추가하고
 ...
 * 값을 put(), get() 할 때 key값을 ""로 감싼 문자열 형식으로 사용해야 하기 때문에
 * 많은 오타로 인한 논리적 오류가 발생할 수 있다.
 * 이러한 문제를 방지하기 위해 key 값을 별도 static 변수로 만들어두고 활용하는 방법을 사용한다
 */
