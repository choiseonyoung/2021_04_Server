...
document.addEventListener("DOMContentLoaded",function() {
	...
	.addEventListener("click",function(ev) {
		...
		if(gb_writer.value == "") {
			...
			// dom.querySelector("input[name='gb_writer']").focus() // 커서를 gb_writer에 포커스를 줘라
			gb_writer.focus();
			// 이벤트 코드 진행을 멈춰라
			return false;
		}
		...
		// false를 리턴하기 때문에 alert저장버튼이 이름 비어있을 때 뜨면 안 됨
		alert("저장버튼"
				+ gb_writer.value + "\n"
				+ gb_email.value + "\n"
				+ gb_password.value + "\n"
...
<body>
	...
	<legend>방명록 쓰기</legend>
		...
		<div>
			<label></label>
			<button class="btn_save" type="button">저장</button> <!-- 원래 submit이라 엔터나 저장 누르면 폼이 자동으로 전송되게 돼있음 button으로 무력화시켜줄 수 있음-->
