...
.addEventListener("click", function(ev) {
		...
		if(className == "btn_home") {
			...
			.location
			.href
			= "${rootPath}/guest/update?gb_seq=" + ${GB.gb_seq}
			...
			if(confirm("방명록을 삭제합니다!!!")) {
				...
				// href는 새로운 페이지를 열고 URL 에서 res한 정보를 보여준다
				// 새로운 페이지에서 뒤로가기를 하면 현재 페이지로 되돌아 올 수 있다.
				
				// replace() 는 현재 페이지에서 URL에서 res한 정보를 보여준다
				// 새로운 페이지에서 뒤로가기를 했을 때 현재 페이지를 볼 수 없다 
				
				document
				.location
				.replace("${rootPath}/guest/delete?gb_seq="+${GB.gb_seq}) // href 아닌 replace라는 함수 사용
			}
		}
	})
})
