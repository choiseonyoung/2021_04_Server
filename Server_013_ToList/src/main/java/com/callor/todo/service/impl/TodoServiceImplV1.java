...
public class TodoServiceImplV1 implements TodoService{
	...
	protected List<Map<String,Object>> select(ResultSet rSet) throws SQLException {
		...
		// 메타데이터 getter
		ResultSetMetaData md = rSet.getMetaData(); // 애드 쓰루스
		
		// Select된 table의 컬럼 개수 getter
		...
		while(rSet.next()) {
			...
			for(int index = 0; index < columns ; index++) {
				...
				// ResultSet으로부터
				// index 위치의 컬럼에 저장된 실제 데이터 getter
	...
	private void viewSelect(ResultSet rSet) throws SQLException {
		...
		// Select 된 table의 컬럼정보 추출
		// 1. 컬럼이 몇개인가
		int columns = md.getColumnCount();
		
		// 2. 메타데이터에서 컬럼의 이름들을 나열하라
		 ...
		 * 실제 데이터와 메타데이터가 모두 포함되어 있따
		 * rSet 객체의 next() method를 한번 호출하면
		 * 	결과 table의 첫번째 레코드를 읽을 수 있도록 준비해준다
		 * 이어서 next() method를 계속해서 호출하면 한줄씩 순서대로 읽을 수 있도록 준비해준다
		 * 만약 더이상 읽을 데이터가 없으면 false return
