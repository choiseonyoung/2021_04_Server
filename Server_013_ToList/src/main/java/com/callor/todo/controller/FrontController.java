 ...
 * 필요할 때마다 URI(URL)을 mapping하여 기능을 수행할 수 있도록 한다
 * 
 * 하지만
 ...
 * POJO
 * (Plan Old Java Object, 어떤 클래스를 상속받지 않은 일반적인 자바 클래스 형식의 코드)
...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// ? 요청을 하면 homecommand한테 역할?을 대신 시키겠다
		// 사용자의 요청이 들어오면 이 객체를 사용하여 처리를 하겠다
		...
		// 사용자로부터 insert라는 요청이 들어오면 이 객체를 사용해서 처리를 해라
	...
	protected void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		// req된 URI 중에서 실제 subPath 부분을 사용하여 처리할 객체를 Map으로부터 추출
		TodoCommand subCommand = commands.get(path); // 처리할 객체가 있는가 추출
		if(subCommand != null) { // 있으면
			// 각 Command 객체의 execute() method에게 실제 요청을 처리하도록 위임하기
			subCommand.excute(req, res);
		}
	}
